(function() {
  ...
  ProjectMilestones = (function(superClass) {
    ...
    function ProjectMilestones() {
      ...
      this.allMergeRequests = bind(this.allMergeRequests, this);
    ...
    ProjectMilestones.prototype.allMergeRequests = function(projectId, milestoneId, fn) {
      var cb, data, params;
      if (fn == null) {
        fn = null;
      }
      this.debug("Projects::Milestones::allMergeRequests()");
      params = {};
      if (params.page == null) {
        params.page = 1;
      }
      ...
      cb = (function(_this) {
        return function(err, retData) {
          if (err) {
            ...
            _this.debug("Recurse Projects::Milestones::allMergeRequests()");
            data = data.concat(retData);
            params.page++;
            return _this.get("projects/" + (Utils.parseProjectId(projectId)) + "/milestones" + (parseInt(milestoneId)) + "/merge_requests", params, cb);
          } else {
            data = data.concat(retData);
            if (fn) {
              return fn(data);
      ...
      return this.get("projects/" + (Utils.parseProjectId(projectId)) + "/milestones" + (parseInt(milestoneId)) + "/merge_requests", params, cb);
    };
